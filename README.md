...
```
.
├── data_counter/               # App export CSVs, one file per Counter participant
├── data_journal/               # App export CSVs, one file per Journal participant
├── data_counter_clean/         # Cleaned Counter CSVs (generated by clean_counter_data.py)
├── out/                        # Generated figures and merged tables
├── counter.csv                 # Counter questionnaire data (semicolon-delimited)
├── journal.csv                 # Journal questionnaire data (semicolon-delimited)
...
No script uses randomness except `FTI_active_days.py`, which runs a permutation test with a fixed seed (`seed=42`). All other outputs are fully deterministic given the input files. Python and package versions should be pinned when archiving for replication.
